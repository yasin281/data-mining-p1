...
dd <- read.csv("filtered_data.csv", sep=",", stringsAsFactors = T);
names(dd)
dim(dd)
summary(dd)
detach(dd)
...
#filamentos/grupos pequeños -> outliers
...
#devuelve a que cluster pertenece el individuo
c1 <- cutree(h1,nc)
...
plot(ShipWeight,OrdQty,col=c1,main="Clustering of credit data in 3 classes")
